...
total_boxes = 0
imgs        = 0
with open('hxb_new' + '.txt', 'w') as ref_detfile: #生成fpp--ref_detfile
    ...
    for im in range (0, len(list_of_imgs)):
        ...
        for k in range (0, len(box)): #
            # print("type(polygons[k]) = ",type(polygons[k]))
            # print('\n')
            # print("polygons[k].shape = ",polygons[k].shape)
            # print('\n')
            # print("polygons[k] = ",polygons[k])
            # print('\n')
            ref_detfile.write(str(list_of_imgs[im]) + ' ' + str(scores[k])+' '+str(box[k])[1:-1]+' '+str(polygons[k].flatten())[1:-1]+'\n') #  hxb
            #ref_detfile.write(' '.join(map(str, polygons[k].flatten()))+'\n')#' '.join(map(str, flattened_matrix))
        ...
        with open(out_path + str(im) + '.txt', 'w') as out_txt: #HXB #新建txt保存顶点
            #browse all boxes
        ...
        cv2.imwrite(out_path+str(im)+'.jpg', img)
